...
API Key Manager (API 密钥管理工具) 是一个 CLI 工具，用于管理和快速切换多个 API 提供商配置。支持 ANTHROPIC_API_KEY 和 ANTHROPIC_AUTH_TOKEN，允许用户在多个 API 提供商之间切换，管理环境变量，并检测配置冲突。
...
- 自动迁移：旧的 `api_token` 模式会自动迁移到新模式
- Token 类型支持：api_key 模式支持 ANTHROPIC_API_KEY 和 ANTHROPIC_AUTH_TOKEN
...
```javascript
{
  ...
  providers: {
    'provider-name': {
      ...
      tokenType: 'api_key' | 'auth_token',  // 仅在 authMode 为 'api_key' 时使用
...
**authMode 和 tokenType 说明**：
- `authMode: 'oauth_token'` - OAuth令牌模式，用于官方 Claude Code
- `authMode: 'auth_token'` - 认证令牌模式，仅设置 ANTHROPIC_AUTH_TOKEN
- `authMode: 'api_key'` - 通用API密钥模式，支持两种 token：
...
4. 在 `bin/akm.js` 中添加 commander 命令定义
